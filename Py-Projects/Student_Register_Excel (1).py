...
import os
import openpyxl
filepath='/home/ubentu/Desktop/My Project/Py-Projects/student.txt'
...
def store():
    ...
    MOBILE=mobile.get()
    #AGE=age.get()
    #print(PIN,NAME,DOB,BRANCH,GENDER,J_D,MOBILE)
    ...
    if PIN and NAME and DOB and BRANCH and GENDER and J_D and MOBILE :
        if os.path.exists(filepath):
            ...
            if fopen:
                fopen.write(PIN+NAME+DOB+BRANCH+GENDER+J_D+MOBILE+"\n")
...
def display():
    #data.insert(tk.END,"")
    fwrite=open(filepath,"r")
    fwrite.seek(0)
    s=fwrite.readline()
    while s:
     
        data.insert(tk.END,s[3])
        s=fwrite.readline()
...
data=Listbox(frame2,width=1260,height=320,bg="#E0FFAF",font='Arial 15')
...
dob=DateEntry(frame1,width=19,font="arial 12",selectmode='day')
...
bra=['CME','ECE','EEE','AI']
branch=ttk.Combobox(root,values=bra,width=21)
...
gender=ttk.Combobox(root,values=gen,width=21)
...
joindate=DateEntry(frame1,width=19,font="arial 12",selectmode='month')
...
mobile=Spinbox(frame1,width=19,font="arial 12")
